...
void main() {
    ...
    // Mouse interaction - create a depression or elevation based on mouse position
    if (mouseDistance < uMouseRadius) {
        float influence = (uMouseRadius - mouseDistance) / uMouseRadius;
        influence = pow(influence, 2.0); // Square for smoother falloff
        pos.z += influence * uMouseStrength;
    }
    
    vWave = pos.z * 0.1; // Pass wave value to fragment shader for coloring
